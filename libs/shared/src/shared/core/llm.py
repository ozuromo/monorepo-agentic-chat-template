...
from shared.schema.models import (
    AllModelEnum,
    AnthropicModelName,
    AWSModelName,
...
def get_model(model_name: AllModelEnum, /) -> ModelT:
    ...
    if model_name in AzureOpenAIModelName:
        ...
        return AzureChatOpenAI(
            azure_deployment=settings.AZURE_OPENAI_DEPLOYMENT_MAP.get(api_model_name),
    ...
    if model_name in AnthropicModelName:
        return ChatAnthropic(model=api_model_name, temperature=0, streaming=True)
    if model_name in GoogleModelName:
        return ChatGoogleGenerativeAI(
            model=api_model_name, temperature=0, streaming=True
        )
    if model_name in VertexAIModelName:
        return ChatVertexAI(model=api_model_name, temperature=0, streaming=True)
    if model_name in GroqModelName:
        if model_name == GroqModelName.LLAMA_GUARD_4_12B:
            return ChatGroq(model=api_model_name, temperature=0.0)
        return ChatGroq(model=api_model_name, temperature=0)
    if model_name in AWSModelName:
        return ChatBedrock(model_id=api_model_name, temperature=0)
    if model_name in OllamaModelName:
        if settings.OLLAMA_BASE_URL:
            ...
            chat_ollama = ChatOllama(model=settings.OLLAMA_MODEL, temperature=0)
        return chat_ollama
